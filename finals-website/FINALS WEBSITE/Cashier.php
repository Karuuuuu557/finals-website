...
$cashier_name = $_SESSION['user_name'] ?? 'Dingdong Khan';
$cashier_role = $_SESSION['role']      ?? 'Cashier/Admin';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'checkout') {
    ...
    $change_given
      ...
      $sl = $conn->prepare("
                INSERT INTO activity_logs (user_id, user_name, type, action, detail, ref, ip_address)
                VALUES (?, ?, 'sale', ?, ?, ?, ?)
            ");
      $sl->bind_param('isssss', $cashier_id, $cashier_name, $log_action, $log_detail, $order_ref, $ip);
      $sl->execute();
...
<body>
  ...
  <div class="main">
    ...
    <div class="order-panel">
      ...
      <div class="order-summary">
        <div class="summary-row"><span>Subtotal</span><span id="subtotalVal">₱0</span></div>        <div class="summary-row" id="discountRow" style="display:none;color:var(--green)">
          <span>Discount</span><span id="discountVal">-₱0</span>
        </div>
        <div class="summary-row total"><span>TOTAL:</span><span id="totalVal">₱0</span></div>
      </div>
  ...
  <script>
    ...
    function updateSummary() {
      ...
      document.getElementById('subtotalVal').textContent = `₱${sub}`;      document.getElementById('totalVal').textContent = `₱${sub - disc}`;
      if (disc > 0) {
        document.getElementById('discountRow').style.display = 'flex';
        document.getElementById('discountVal').textContent = `-₱${disc}`;
      }
    }
    ...
    function openCheckout() {
      ...
      ['custFirst', 'custLast', 'custAddress', 'custContact'].forEach(id => document.getElementById(id).value = '');
    ...
    async function confirmCheckout() {
      ...
      try {
        ...
        // REPLACE WITH:
        if (data.success) {
          // Update local PRODUCTS stock to match DB (re-fetch)
          try {
